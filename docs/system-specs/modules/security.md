...
**Windows UNC trusted-root gate** (`unc_probe_allowed` + `validate_file_path` in `hooks.py`) -- a UNC path names a HOST, so resolving or stat-ing untrusted UNC-shaped text (`\\evil\share\x.png` or `//evil/share/x.png` echoed in any message or query) makes Windows open an outbound SMB connection to an attacker-named host. `validate_file_path` therefore consults `unc_probe_allowed` **before any resolution** on Windows -- the ordering is the control, since `realpath` on UNC text is itself the probe -- and the gate's comparison is purely lexical (`normcase`/`normpath`), never touching the network. Filesystem access is restricted to UNC paths under four trusted roots, all admitted on the same two properties -- the root is derived from the **environment** (`Path.home()` or an explicit `KIRO_HOME`), never from message text or a request parameter, so an attacker cannot steer the prefix at `\\evil\share`; and the gateway **already reaches that host by design**, so admitting the root adds no new SMB destination: (1) the crew **data home** -- on a roaming profile the home directory is itself a UNC share, the one legitimate source of UNC attachment paths; (2) the **temp directory** -- channel-side image staging; (3) the **kiro agents directory** (`<kiro home>/agents`) -- `apps.bridges._register_agents` and `agent.rebuild_agent_config` write the managed specs there, and it is a *sibling* of the data home on the same share, so before #6721 its absence made `_read_agent_spec` silently read every user-level agent spec as absent on a UNC home; (4) the **kiro-cli transcript directory** (`<kiro home>/sessions/cli`) -- read by design on every platform (`session_map`, `acp/*`, `providers/acp`, `dashboard/handlers/usage`), so before #6733 its absence made the usage page silently count zero sessions on a UNC home. Root (4) is **not** gateway-written -- kiro-cli writes it -- which is why #6733 held it back from #6728 for its own decision; the conclusion was that a foreign writer controls a file's NAME and CONTENT under an already-fixed prefix, and neither is what this gate covers (content is the reader's problem -- `is_sensitive_path`, then parsing -- and a name cannot introduce a host). Admission is the transcript directory **exactly**, not the kiro home: `<kiro home>/settings/mcp.json` decides which MCP servers exist and has no reader needing the gate opened, so it stays refused. Because the gate is lexical on the RAW path it cannot see where a LINK points, and `validate_file_path` resolves after admitting -- so the transcript reader (`usage._parse_sessions`) refuses a symlink or Windows junction via `pinned_fs.is_reparse_point` **before** calling the validator, since `lstat` reads only the link inode on the already-trusted share while `realpath` would contact the target's host. kiro-cli writes transcripts as regular files, so nothing legitimate is refused. Note this link-through-a-trusted-root property is **not** specific to the transcript root: it holds for the data home, the temp dir and the agents dir too, and closing it inside the gate would require rejecting reparse points before resolution for every consumer -- tracked separately rather than folded in here. The prefix comparison is separator-boundary-anchored (a sibling share on the same host, or an `agents-evil` / `cli-evil` neighbour directory, is refused), a root that is not itself UNC-shaped admits nothing (the roots cannot become a bypass on an ordinary local home), and the two kiro roots are **memoized together per configuration** (keyed on the raw `KIRO_HOME`, the override hook, and both accessor identities): `kiro_agents_dir()` and `kiro_sessions_dir()` both route through `kiro_home()`, which resolves `KIRO_HOME` with filesystem I/O -- on a UNC-shaped override, an SMB touch -- so they must not run per gate check on hot/async validation paths. Each accessor is tolerated **independently** and a failure memoizes that root as absent fail-safe, so one broken resolution neither takes the gate down nor costs its sibling a root (recovery is an env change or restart, and the degraded state is the pre-#6721/#6733 status quo). The **project-level** agents dir (`project_agents_dir`) is deliberately NOT admitted: an arbitrary project directory is neither environment-derived nor a host the gateway already reaches, so admitting it would be a genuine trust-boundary widening rather than a repair.
