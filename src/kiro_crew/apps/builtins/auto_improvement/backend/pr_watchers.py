    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            return "", (
                ...
                f"{redact_and_truncate((proc.stderr or '').strip(), 200)}"
            )
        if branch:
            ...
            if checkout.returncode != 0:
                ...
                return "", (
                    ...
                    f"{redact_and_truncate((checkout.stderr or '').strip(), 160)}"
                )
...
def publish_if_authorized(pr: str, status: dict[str, Any]) -> tuple[bool, str]:
    ...
    if proc.returncode != 0:
        ...
        return False, f"gh pr ready failed: {redact_and_truncate(tail[0], 160)}"
