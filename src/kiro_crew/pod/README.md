...
kirocrew pod up   <wt> --seed crons-active  # pre-populate its HOME from a named scenario
...
```bash
kirocrew pod up my-wt --seed crons-active
...
Named fixtures are copied into a sibling staging directory and renamed into
place only after the copy completes. Their config is then forced through
`SEED_DISABLED_SECTIONS`, so tunnel and channel enablement cannot survive the
seed. A populated home is never overwritten or re-seeded; service restarts keep
the sessions and logs already present. After health succeeds, `pod up` reads the
fixture marker back from the home and fails if the requested scenario did not
land.
