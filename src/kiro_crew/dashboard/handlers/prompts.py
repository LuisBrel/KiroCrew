...
def _local_prompt_entry(name: str, project_dir: Path | None) -> dict[str, Any] | None:
    """The single local prompt *name* can select, by exact stem — no directory scan.

    ``_find_prompt`` runs on the chat turn's own thread when ``@mention`` is
    expanded, so resolving "local" by enumerating ``<project>/.kiro/prompts``
    would put a glob plus a read per file on the event loop for every turn that
    starts with ``@``, in a directory the gateway does not own and that may be
    network-backed. A local entry's ``name`` and ``fullName`` are both the file
    stem, so exactly one path in that directory can ever match: build that path
    and gate it. Bounded at one open regardless of how many files the project
    keeps there.

    ``_plain_stem_ok`` gates the name BEFORE it is joined to anything, the same
    order the scoped read applies it in: a name that is not a plain component
    never reaches the filesystem at all, so ``@sub/foo`` and ``@../escape`` are
    misses rather than paths that have to be resolved and refused afterwards.
    ``glob('*.md')`` yields the listing direct children only, and the same
    predicate is applied there, so what this can address and what a listing shows
    are the same set.

    The entry itself is minted by the parent package's ``_prompt_dir_entry``, the
    same gate the listing scan mints through, so a name resolved this way and a
    name resolved by the scan are refused on identical grounds.
    """
    if project_dir is None or not _plain_stem_ok(name):
        return None
    import kiro_crew.dashboard.handlers as _pkg

    prompts_dir = Path(project_dir) / ".kiro" / "prompts"
    try:
        return _pkg._prompt_dir_entry(prompts_dir / f"{name}.md", prompts_dir, "local")
    except (OSError, ValueError):
        # A name carrying an embedded NUL (``%00`` in the URL path) makes path
        # resolution raise ``ValueError``: no match, never an unaudited 500.
        return None
...
def _find_prompt(raw_name: str, project_dir=None) -> dict[str, Any] | None:
    ...
    :func:`_local_prompt_entry`, which costs one open instead of a scan. Ordering
    ...
    on-loop ``@mention`` path free of an unbounded, uncacheable directory walk.
...
async def api_prompt_detail(request: web.Request) -> web.Response:
    ...
    # offload it so a slow FS can't stall the event loop. Resolve the local
    # project on the loop first and capture it into the executor job, through the
    # same _prompt_local_project seam the lister uses, so an unscoped lookup can
    # only match a local prompt the same request would have been shown.
    state: DashboardState = request.app["state"]
    session_key = _read_session_key(request)
    project_dir = _prompt_local_project(request, state, session_key)
