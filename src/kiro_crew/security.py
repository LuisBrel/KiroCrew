...
def _build_sensitive_regex() -> re.Pattern[str]:
    ...
    # stripped dot on the home's last segment), and the resolved home literal
...
def is_sensitive_bash_command(command: str) -> str | None:
    ...
    """
    # ── Pass 1: regex fast-path ──
    # Cap the raw-text scan window. The Windows-native-spelling branch of the
    # sensitive-path regex contains a ``(?:…)*`` group whose alternation is
    # retried at every offset, making a full-input scan O(n²) — a ~24 KB
    # single command drove it past the synchronous-hook watchdog and
    # terminated the gateway (found in review). This reuses the same
    # deliberate window cap the user-custom-regex O(n²) path already applies
    # (``_DENY_FALLBACK_SCAN_MAX_CHARS``; see security.md and the
    # ``_whole_re.search(text[:cap])`` site above): the project avoids an
    # RE2 dependency, no real credential-path command approaches the cap, and
    # Pass 2's tokenizing normalizer (which is NOT regex-quadratic and routes
    # every path token through ``is_sensitive_path``) is the backstop for any
    # sensitive path sitting beyond the window.
    scan = command[:_DENY_FALLBACK_SCAN_MAX_CHARS]
    if _get_sensitive_re().search(scan):
    ...
    if _RELATIVE_SENSITIVE_RE.search(scan):
