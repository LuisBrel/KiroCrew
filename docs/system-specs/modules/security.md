...
- **Run coordinator ledger (`run-coordinator/`)** — the directory contains canonical run, command, lease, and delivery metadata plus SQLite WAL/SHM/journal sidecars. It is on `_CREW_SECRET_LEAVES` as a whole directory so an agent cannot read task payloads, forge terminal delivery, or rewrite its own desired/observed state and fencing metadata. Every Kiro Crew OS-sandbox mode resolves the active data home and denies reads, writes, and hardlinks to its ledger, including when `KIROCREW_HOME` places it outside `Path.home()`; the default and legacy lexical paths remain covered too. An ordinary directory uses its configured coordinator path directly. When the data-home path traverses a link, a canonical directory record under the owner-only `~/.kirocrew.run-coordinator/` anchor directory is keyed by the lexical home selection and shared by SQLite and sandbox construction. The anchor is itself hidden by every Kiro Crew OS sandbox and is on `_SENSITIVE_HOME_DIRS`, so retargeting the link cannot redirect the next gateway process to attacker-prepared state. A retargeted link intentionally leaves SQLite on the persisted canonical ledger, while hook-layer path matching follows the current resolved data home; every execution path that would otherwise run without an OS sandbox therefore compares those identities and fails closed when they differ. The macOS Seatbelt profile denies both the lexical and canonical spellings of the stable anchor, and also denies writes to the active ledger path and each rename-sensitive ancestor as literal targets, so a symlinked home or renamed parent cannot move protected state outside the path-based subtree rules. The Linux namespace path creates and fail-loud owner-tightens both the anchor directory and the active ledger directory before building the launcher. Creating the anchor before the launcher's existence-guarded bind-mask loop closes the pre-seed window in which an agent could otherwise materialize a future trusted anchor record. The launcher derives the ledger bind mask and rename guards from the canonical spelling, then self-binds every canonical ancestor below the filesystem root as a writable mountpoint (outermost first) before bind-masking the ledger. The mountpoints block ancestor renames without making ordinary app state under the data home read-only. This closes raw subprocess access that bypasses tool-call path matching. The directory, optional anchor record, and known database files are owner-only on creation and reopen.
  Existing coordinator anchor records are consulted before the ordinary-directory fast path, so replacing a selected data-home link with a directory cannot abandon the prior ledger on restart. Seatbelt applies the ledger read, write, hardlink, and rename-ancestor denies to both the configured lexical spelling and the persisted canonical spelling.
