...
def setup_safe_clone(url: str, scratch_root: Path, *, timeout_s: int = 300) -> tuple[dict, str]:
    ...
    if proc.returncode != 0:
        ...
        return {}, f"git clone failed: {redact_and_truncate(tail[0], 200)}"
    if not _repository_is_safe(dest):
        rmtree_force(dest)
...
def list_clone_branches(clone: Path, *, timeout_s: int = 30) -> tuple[list[str], str]:
    ...
    if proc.returncode != 0:
        tail = (proc.stderr or "").strip().splitlines()[-1:] or [""]
        return [], f"could not list branches: {redact_and_truncate(tail[0], 160)}"
    names: list[str] = []
    seen: set[str] = set()
...
def checkout_branch(clone: Path, branch: str, *, timeout_s: int = 120) -> tuple[bool, str]:
    ...
    if fetched.returncode == 0:
        ...
        return False, f"could not check out {bare}: {redact_and_truncate(err[0], 160)}"
    ...
    return False, f"could not fetch {bare}: {redact_and_truncate(err[0], 160)}"
