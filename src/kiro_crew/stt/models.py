...
def _public_addresses(host: str, port: int, label: str) -> list[str]:
    ...
    *label* is the already-redacted text to name in a refusal. A caller must never
    ...
    if not addresses or any(address_is_not_public(address) for address in addresses):
        # Named, and with the way out. This message reaches the operator verbatim in
        # Settings > Voice ("The download failed: {{error}}"), and a LAN mirror is a
        # core audience for a self-supplied model, so a bare mechanism sentence made
        # every attempt fail identically until they found the env var in the docs.
        raise ModelDownloadError(
            f"refusing a non-public address for: {label} -- to use a mirror on a "
            f"private network, name its origin in {CUSTOM_MODEL_ALLOW_PRIVATE_ENV}"
        )
    return addresses
...
class _PinnedAddressHTTPSConnection(http.client.HTTPSConnection):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        # Set per request by `_ScreenedHTTPSHandler.https_open`. False here so an
        # instance built any other way screens, rather than defaulting to the exemption.
        self.declared_private_origin = False
    ...
    def _connect_to_a_screened_address(
        ...
        if tunnel_host:
            ...
            raise ModelDownloadError(
                ...
                "by the proxy, so it cannot be screened at the socket -- fetch a "
                "custom model on a connection that does not use https_proxy, or "
                f"name the mirror's origin in {CUSTOM_MODEL_ALLOW_PRIVATE_ENV}"
            )
        if self.declared_private_origin:
            # Past the tunnel refusal, so a proxied install is still refused here;
            # what is lifted is only the requirement that the address be PUBLIC, for
            # the one origin the operator named in `CUSTOM_MODEL_ALLOW_PRIVATE_ENV`.
            # The address is left to the resolver, which is the transport this path
            # had before the screen existed -- the air-gapped mirror's own case.
        ...
        # Every entry is screened public, so trying them in turn keeps stock
        # `create_connection`'s try-each-address behaviour without widening the pin:
        # binding only `approved[0]` made a host with several A records -- or one
        # whose AAAA is listed first on an install with no working IPv6 route --
        # fail the download outright, where the catalog path (plain connection, no
        # screen) falls back. The last error is the one raised, as the stdlib does.
        ...
        for candidate in approved:
            try:
                ...
                last_error = exc
        raise last_error if last_error else OSError("no screened address to connect to")
...
class _ScreenedHTTPSHandler(urllib.request.HTTPSHandler):
    ...
    def https_open(self, req: urllib.request.Request) -> http.client.HTTPResponse:
        if not getattr(req, "screen_address", False):
            return self.do_open(http.client.HTTPSConnection, req, context=self._screen_context)
        # A hop on the operator's declared private origin is exempt from the
        # public-address requirement, and from nothing else: it still rides this
        # connection, so the proxy-tunnel refusal still reaches it. The flag travels
        # on the request because only the redirect handler knows whether the chain
        # has left that origin, and that answer cannot be re-derived from the host
        # the socket is handed -- a chain that went public first and was then aimed
        # back at the declared host would look identical here.
        exempt = getattr(req, "declared_private_origin", False)

        def _screened_connection(*args: Any, **kwargs: Any) -> _PinnedAddressHTTPSConnection:
            conn = _PinnedAddressHTTPSConnection(*args, **kwargs)
            conn.declared_private_origin = exempt
            return conn

        return self.do_open(_screened_connection, req, context=self._screen_context)
...
class _HttpsOnlyRedirectHandler(urllib.request.HTTPRedirectHandler):
    ...
    def redirect_request(
        ...
        if hop is not None:
            ...
            #
            # The one hop that is NOT pinned is a same-origin hop on the private
            # origin the operator declared in `CUSTOM_MODEL_ALLOW_PRIVATE_ENV`. The
            # pin refuses a private address outright (`_public_addresses`), and that
            # origin's own initial request is already exempt in `_urlopen` -- so
            # pinning the hop refused the air-gapped mirror the moment it answered
            # `/model -> /blob/...`, an ordinary artifact-store shape, leaving the
            # escape hatch working only for a mirror that never redirects.
            #
            # `crosses_origin` is what keeps that narrow, and it is sticky: a chain
            # that has left the operator's host can never claim this back, and a
            # `Location` naming any OTHER private origin crosses and is already
            # refused eagerly above. So this admits no address the operator did not
            # name themselves.
            stays_on_the_declared_private_origin = not crosses_origin and (
                _private_custom_url_is_allowed(newurl)
            )
            # Marked either way, so the hop cannot escape the pinned connection and
            # the proxy-tunnel refusal with it; the exemption rides its own flag and
            # lifts only the public-address requirement.
            hop.screen_address = True  # type: ignore[attr-defined]
            hop.declared_private_origin = (  # type: ignore[attr-defined]
                stays_on_the_declared_private_origin
            )
            if crosses_origin:
...
def _urlopen(url: str, timeout: float, *, screen_addresses: bool = False) -> IO[bytes]:
    ...
    :meth:`_PinnedAddressHTTPSConnection._connect_to_a_screened_address`. That holds for
    the operator's declared private origin as well: it is exempt from the PUBLIC-address
    requirement, not from the tunnel refusal, because the far end of a tunnel is chosen
    by the proxy and an exemption naming one origin cannot vouch for it.
    ...
    if screen_addresses:
        # The declared private origin is EXEMPT from the public-address requirement,
        # not from screening: the request is marked either way, so it rides the
        # pinned connection and the proxy-tunnel refusal there still applies to it.
        # Leaving it unmarked -- which this did -- put it on the plain connection, so
        # on an `https_proxy` install urllib tunnelled it and the refusal this
        # docstring documents never ran; the exemption then covered whatever the
        # proxy would CONNECT to rather than the one origin the operator named.
        declared_private = _private_custom_url_is_allowed(url)
        if not declared_private:
            # The CONFIGURED address is screened too, not just the hops after it. It
            # arrives from `stt.custom_model_url` in `config.json` -- agent-reachable,
            # and write-protected only at the file-edit tool gate -- so it is not the
            # operator-only value `MODEL_URL_ENV` is: an injected agent that wrote a
            # private https address there had the gateway issue an unauthorized
            # internal GET on the next transcription. The sha256 pin does not stop
            # that, because the request is made before any digest is computed and the
            # side effect on the internal service has already happened by the time
            # the body is discarded.
            #
            # BOTH halves, for the reason `_refuse_non_public_host` states: the eager
            # lookup produces a clear refusal before a socket is opened, and the pin
            # is what closes the DNS-rebind window between the lookup and the connect.
            #
            # An operator who genuinely mirrors weights internally names that origin
            # in `CUSTOM_MODEL_ALLOW_PRIVATE_ENV` and gets the address requirement
            # lifted for that origin on this hop only -- not for any other private
            # address, which a boolean switch would also have admitted through the
            # same config value. Redirect screening is unaffected either way.
        ...
        request.declared_private_origin = declared_private  # type: ignore[attr-defined]
        target = request
    try:
