"""Redact-before-bound on subprocess stderr in the dev_fleet server.
...
class TestRebaseConflictTailIsRedactedBeforeTheBound:
    ...
    async def _rebase_with_output(
        ...
        # crafted output, and the abort succeeds.
        ...
        monkeypatch.setattr(mod, "_run_cmd", _run_cmd)

        async def _remote() -> str:
            return "origin"

        monkeypatch.setattr(mod, "_upstream_remote", _remote)
        out = await mod._rebase_locked({"path": str(tmp_path)})
