...
NON_EGRESS_REDACTION_MODULES: frozenset[str] = frozenset(
    {
        ...
        # Source-side pre-passes, same shape as the aws_control scrubbers: subprocess
        # stderr that can carry an authenticated URL (git echoes the remote's userinfo
        # on an auth failure; pip echoes an authenticated index URL) is scrubbed with
        # redact_and_truncate at the point each error string is BUILT, so a fixed
        # character bound can never cut a credential mid-match (a sliced fragment no
        # longer matches the credential regex, and a downstream pass cannot catch it).
        ...
        # - clone_setup.py's and deps.py's error strings reach the dashboard only
        #   through the backend routes (the registered sink for this app).
