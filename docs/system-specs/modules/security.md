...
- **Frontend mirror** (`website/src/utils/sanitize.ts::sanitizeExfiltrationUrls`, issue #7820): the browser-side redactor mirrors the SHAPE of `_exfil_url_warning`'s layering rather than its wiring. It splits the checks into an UNCONDITIONAL tier — `EXFIL_PERCENT_RE` (heavy percent-encoding, 20+ consecutive octets, mirrors `_EXFIL_PERCENT_RE`) plus `EXFIL_CREDENTIAL_RE` (AWS key id / SSH-or-PEM header / Slack token, the hard markers) — applied to **every** host, and a HEURISTIC tier (base64 blob `EXFIL_B64_RE` + aggregate query length ≥ 200) that is skipped ONLY for a narrow, **code-owned exact-host allowlist** `EXFIL_EXEMPT_HOSTS = {github.com, monitorportal.amazon.com}` — the two confirmed-benign false-positive hosts from the issue (`github.com/.../issues/new?title=…&body=…`, `monitorportal.amazon.com`). The allowlist is matched **case-insensitively and EXACTLY** (the captured host is lowercased and tested with `Set.has`), never by suffix, so a look-alike like `github.com.evil.example` is NOT exempt. It is intentionally tiny and hard-coded (not config-driven, not agent-writable) because the frontend has no access to the backend's companion-supplied `_exempt_exact_hosts()` at render time and must not let a model widen its own trust. Unlike the backend the frontend does NOT diverge in the JWT credential parity (pinned by `test/test_redaction_mirror_parity.py`) and introduces **no regex lookbehind** (the file is in the eagerly-loaded entry chunk; at the `safari14` esbuild target a lookbehind literal is rewritten to `new RegExp()` and throws at module load, blanking the dashboard). Covered by `website/src/test/sanitizeExfiltrationUrls.test.ts`. **Accepted residual risk:** because the base64-blob and aggregate-length heuristics are skipped for the exempt hosts, a sub-200-char query carrying a base64 blob at `github.com` / `monitorportal.amazon.com` renders unredacted (the unconditional tier catches only the LITERAL `AKIA…`/SSH/PEM/Slack markers, not percent-encoded or base64 payloads, since the frontend does not percent-decode) — this is a deliberate tradeoff because the guard is display-only (the agent can already fetch any URL via tools) and a two-host code-owned list is far narrower than exempting all https URLs, and it mirrors the same structural property the backend's own host-exemption tier has. The exempt-host match is **port-agnostic** (`github.com:443` is also exempt because the port is captured separately and never checked), intentionally matching the backend's plain host-exemption tier (`_dom in exempt_hosts`, which does not gate on port), unlike the OAuth/Slack carve-outs that do refuse an explicit port.
