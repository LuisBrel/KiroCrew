...
- **The flag is operator-only on the agent side too.** The builtin agent
  deny rule `self-protection-dev-mode-out-of-root-confirm` refuses any agent
  shell command carrying the flag — matched both as literal text and, via the
  ...
  spelling. An auto-approved Bash tool therefore cannot supply the attestation
  itself — the confirmation must come from
  the operator's own terminal, which the agent deny rules do not govern. The
  `dev` subparser is built with `allow_abbrev=False`, so argparse rejects
  abbreviated spellings (`--confirm`) that would otherwise reach the flag
  without its literal text ever appearing.
