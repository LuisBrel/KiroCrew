...
@pytest.mark.asyncio
async def test_refresh_holds_the_sync_gate_so_the_next_sync_cannot_race_it(monkeypatch):
    """The two halves of the pair must describe the same moment.

    If the NEXT sync could begin (fetch and `merge --ff-only` included) while the
    derivation is still reading, the pair would name the new HEAD's website/ tree
    with a bundle the previous one built -- and if that sync failed before its own
    build, the forged pair would stand: a later sync landing the same tree skips and
    serves a bundle no build of that tree produced. `_SYNC_LOCK` is one of the two
    things that prevent it, by serializing the derivation against a sync's assembly
    (the gate and the spawn both run under that lock); the other is the ordering
    pinned by the sibling test below.

    Asserted by observing the lock DURING the derivation, since the ordering is the
    whole property and a test that only checks the result would pass either way.
    """
    ...
    assert held == [True], "the derivation ran without the sync gate held"
    # And the gate is released afterwards, or the next sync would hang forever.
