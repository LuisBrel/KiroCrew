...
def test_probe_evict_drops_the_entry_and_its_dir(monkeypatch, tmp_path) -> None:  # type: ignore[no-untyped-def]
    monkeypatch.setattr(routes, "_PROBE_CACHE", {})
    pdir = tmp_path / "dc-probe-e"
    pdir.mkdir()
    routes._probe_put("h", str(pdir), {"/": "/x/a.png"}, "/proj/dist", "sig-a")
    routes._probe_evict("h")
    assert routes._probe_get("h") is None
    assert not pdir.exists()
    # Evicting an unknown or falsy handle is a no-op, not an error.
    routes._probe_evict("nope")
    routes._probe_evict("")
