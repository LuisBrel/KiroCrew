...
def seed_home_from_scenario(cfg: PodConfig, name: str, scenario: str) -> bool:
    ...
    # Pods cannot run on Windows, but these pure-function tests still collect
    # there. Keep the unsupported platform's legacy copy path testable; every
    # production pod backend has the descriptor primitives required below.
    if not IS_POSIX:  # pragma: no cover - exercised by Windows CI
        previous = os.environ.get("KIROCREW_HOME")
        os.environ["KIROCREW_HOME"] = str(home_dir)
        try:
            seed(scenario)
...
def seeded_scenario_in_home(cfg: PodConfig, name: str) -> str | None:
    ...
    if not IS_POSIX:  # pragma: no cover - pods are unavailable on Windows
        manifest = home / seed_mod.FIXTURE_MANIFEST
        try:
            return _fixture_name_from_manifest_text(manifest.read_text(encoding="utf-8"))
        except OSError:
            return None
    try:
