...
class AgentRunner:
    ...
    def run(
        ...
        *,
        ...
        if proc.returncode != 0:
            ...
            # redact_and_truncate's head bound) because the END of stderr carries the
            # actionable error; slicing redacted text can at worst split a marker.
            return AgentResult(
                ok=False,
                error=f"exit {proc.returncode}: {redact(proc.stderr or '')[-400:]}",
                duration_s=dur,
            )
        try:
    ...
    def _run_streaming(
        ...
        stderr_tail = redact("".join(stderr_chunks))[-400:]
