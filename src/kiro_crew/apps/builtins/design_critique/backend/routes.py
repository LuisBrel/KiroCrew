...
def _probe_put(
    ...
    if not handle:
        return
    # _discover_from_dir evicts the handle before it starts, so on the ordinary path
    # there is nothing to replace. This drop covers the one case that leaves: two
    # discoveries of the same stable "local:<path>" handle interleaving, where the
    # second store would otherwise orphan the first's dir until the TTL sweep. rmtree
    # is blocking IO; this runs inside the discover to_thread worker, so it stays off
    # the event loop.
...
def _probe_evict(handle: str) -> None:
    # Drop a handle's entry AND its retained dir. Called when a discovery installs no
    # reusable entry: a prior entry must not survive as a fallback, or /render would
    # serve the earlier capture for a project whose discovery just failed — a gate
    # appeared on every screen, the probe timed out, the build output went away. The
    # old token can still match in those cases, so nothing downstream would notice.
    # rmtree is blocking IO, so callers on the event loop must reach this through
    # asyncio.to_thread.
    if not handle:
        return
    with _PROBE_CACHE_LOCK:
...
async def _discover_from_dir(directory: Path, handle: str) -> dict[str, Any]:
    ...
    # Re-discovering a handle supersedes whatever was cached under it, so evict FIRST
    # and let a successful probe install the replacement. Evicting on the way out
    # instead would be skipped by every path that does not reach the end: a timeout, a
    # node/git failure, an early return, an exception the job wrapper catches. A stale
    # entry surviving any of those is invisible downstream, because its token can still
    # match and /render would serve the earlier capture for a project whose discovery
    # just failed. A repo handle is a fresh clone id, so this is a no-op there.
    await asyncio.to_thread(_probe_evict, handle)
    ...
    if routes:
        ...
        for s in probe.get("screens") or []:
            ...
            png = s.get("path")
            if s.get("route") is not None and png and not s.get("overlay"):
        ...
        #
        # The probe deliberately stays WITHOUT --full while /render runs WITH it, so a
        # reused probe PNG is a viewport capture and a covered pick loses below-the-fold
        # content. The reason is blast radius, not screenshot cost: this ONE subprocess
        # renders up to 20 routes of an unknown project under a single shared
        # _CAPTURE_TIMEOUT, and a full-page screenshot of an unknown page is unbounded
        # (a long or virtualised list can make it enormous or very slow). One
        # pathological route would then exhaust the shared budget, the manifest would
        # not parse, and EVERY route would come back canSee=False — losing all of
        # discovery to improve a subset of renders. /render's --full runs on the two or
        # three routes the user picked, with the same budget and far more headroom per
        # route, which is why the flag belongs there and not here.
