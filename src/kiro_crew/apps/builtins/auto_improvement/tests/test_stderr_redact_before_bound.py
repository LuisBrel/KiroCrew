...
recognise it either. The fix is ``security.redact_and_truncate``, which scrubs
...
class TestNoRawBoundedStderrSliceAnywhereInTheApp:
    ...
    bound must run ``redact_and_truncate`` instead.
    """
    ...
    # The sanctioned tail form: redact the FULL text, then cut. A tail cut of
    # already-redacted text can at worst split a redaction marker, never a secret.
    _SANCTIONED_TAIL = re.compile(r"redact(?:_and_truncate)?\([^\n]*\)\s*\[-\d+:\]")
    ...
    def test_no_module_slices_stderr_before_redaction(self) -> None:
        ...
        assert offenders == [], (
            ...
            "use security.redact_and_truncate(text, bound) instead: " + "; ".join(offenders)
        )
