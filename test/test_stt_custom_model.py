...
def test_the_stt_refusals_use_the_one_shared_redactor():
    ...
    # embeddings holds no copy and no re-export: its own download path redacts
    # through `asset_downloader` now, and `kirocrew doctor` reaches this module
    # directly. Asserting ABSENCE rather than identity is what stops a copy
    # reappearing under the old name.
    assert not hasattr(embeddings, "redact_model_url")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # `urlsplit` strips only tab/CR/LF, so any other control byte a redirecting
        # server puts in `Location` survives into `hostname` — and this value is
        # printed to an operator's terminal. An ESC repaints it, a BEL rings it, a
        # bidi override reverses the host it appears to name: refuse the value.
        ("http://\x1bc" + "models.example/m.bin", "<unparseable URL>"),
        ("http://models\x07.example/m.bin", "<unparseable URL>"),
        ("https://models.example\u202e/m.bin", "<unparseable URL>"),
        # A non-ASCII host is not a control byte: an IDN mirror still gets named.
        ("https://m\u00fcnchen.example/m.bin", "https://m\u00fcnchen.example"),
...
def test_a_dead_first_address_falls_back_to_the_next_screened_one(monkeypatch):
    """A screened host with several addresses keeps stock connect's try-each behaviour.

    Pinning the address must not also pin us to the FIRST one: a host with two A
    records whose first server is down, or an AAAA listed ahead of an A on an install
    with no working IPv6 route, would fail the download outright while the catalog
    path (plain connection, no screen) falls back. Every candidate here is already
    screened public, so trying the next weakens nothing.
    """
    dead, alive = "93.184.216.35", _PUBLIC_ADDRESS

    def _two_answers(host, port, *a, **kw):
        return [(0, 0, 0, "", (dead, port)), (0, 0, 0, "", (alive, port))]

    monkeypatch.setattr(models.socket, "getaddrinfo", _two_answers)
    connection, connected, _wrapped = _pinned(monkeypatch)
    real_create = models.socket.create_connection

    def _first_one_is_down(address, timeout=None, source_address=None):
        if address[0] == dead:
            connected.append(address)
            raise OSError("connection refused")
        return real_create(address, timeout, source_address)

    monkeypatch.setattr(models.socket, "create_connection", _first_one_is_down)
    connection.connect()
    assert connected == [(dead, 443), (alive, 443)], "the dead address must not end the attempt"
...
def test_the_declared_private_mirrors_own_hop_keeps_the_pin_but_not_the_address_bar(monkeypatch):
    """The escape hatch has to survive the mirror's first redirect, or it is not one.

    An operator who names their private origin in ``CUSTOM_MODEL_ALLOW_PRIVATE_ENV``
    must be able to fetch from it. Screening that hop like any other took that back:
    the address check refuses a private address outright --
    ``test_a_host_answering_with_a_public_and_a_private_address_is_refused`` is the
    proof that ``_public_addresses`` is a refusal and not just a bind -- so an artifact
    store answering ``302`` to its own blob path failed anyway, and the override only
    worked for a mirror that never redirects.

    What the hop gets back is the ADDRESS bar being lifted, and not the pin itself:
    ``screen_address`` stays set so the hop still rides
    ``_PinnedAddressHTTPSConnection``, whose proxy-tunnel refusal must reach the
    declared origin too -- an exemption naming one origin cannot vouch for whatever a
    proxy would CONNECT to. Asserted on the two attributes because they are the whole
    decision: ``https_open`` reads the first to choose the connection and passes the
    second to it.

    The second half is the control that keeps the hole shut: the SAME hop, with the
    override unset, must not be exempt. Delete the ``_private_custom_url_is_allowed``
    call and that half still passes while the first fails; invert the condition and
    the control fails.
    """
    _resolves_to(monkeypatch, "10.0.0.7")
    same_origin_blob = _URL.rsplit("/", 1)[0] + "/blob/abc"

    monkeypatch.setenv(models.CUSTOM_MODEL_ALLOW_PRIVATE_ENV, "https://models.example")
    handler = models._HttpsOnlyRedirectHandler(screen_addresses=True)
    first = urllib.request.Request(_URL)
    first.timeout = None
    declared = _hop(handler, first, same_origin_blob)
    assert declared is not None, "the declared mirror's own hop must still be followed"
    assert (
        getattr(declared, "screen_address", False) is True
    ), "the hop stays pinned, so the proxy-tunnel refusal still reaches it"
    assert getattr(declared, "declared_private_origin", False) is True, (
        "a hop that stays on the origin the operator declared must be exempt from the "
        "public-address requirement its configured url was exempt from"
    )
    assert getattr(declared, "left_operator_origin", False) is False

    monkeypatch.delenv(models.CUSTOM_MODEL_ALLOW_PRIVATE_ENV)
    undeclared = _hop(handler, first, same_origin_blob)
    assert getattr(undeclared, "screen_address", False) is True
    assert (
        getattr(undeclared, "declared_private_origin", False) is False
    ), "without the override the same hop stays fully screened"


def test_a_declared_private_origin_is_still_refused_through_a_proxy_tunnel():
    """The exemption lifts the address bar, never the tunnel refusal.

    Leaving the declared origin's request UNMARKED gave it the plain connection, so on
    an ``https_proxy`` install urllib tunnelled it and the refusal ``_urlopen``'s
    docstring promises never ran -- and the exemption then covered whatever the proxy
    chose to ``CONNECT`` to, rather than the one origin the operator named. The far end
    of a tunnel is resolved by the proxy, so there is no address here to screen and no
    reason to believe it is the declared one.

    Driven at the ``_create_connection`` hook because that is where both branches meet:
    the tunnel check must run BEFORE the exemption is consulted.
    """
    conn = models._PinnedAddressHTTPSConnection("models.example")
    conn.declared_private_origin = True
    conn._tunnel_host = "proxy.example:3128"
    with pytest.raises(models.ModelDownloadError, match="through an https proxy"):
        conn._create_connection(("proxy.example", 3128), None, None)
...
def test_an_operator_can_re_allow_a_private_custom_mirror_by_environment(monkeypatch):
    ...
    with models._urlopen(_URL, timeout=1.0, screen_addresses=True):
        pass
    assert len(seen) == 1 and seen[0].full_url == _URL, "the mirror is still reached"
    # Marked, so the request keeps the pinned connection and its proxy-tunnel refusal;
    # exempt, so that connection does not require a PUBLIC address. Handing the plain
    # string through instead is what silently dropped the tunnel refusal.
    assert getattr(seen[0], "screen_address", False) is True
    assert getattr(seen[0], "declared_private_origin", False) is True
