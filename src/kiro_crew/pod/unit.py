...
def _write_dropin_atomic_nofollow(dst: Path, content: str) -> None:
    """Atomically publish *content* without following a planted link.

    systemd pod drop-ins are Linux-only in production. The POSIX path pins the
    complete parent chain, creates a private no-follow temporary through that
    directory descriptor, and replaces the final name relative to the SAME
    descriptor. A final symlink seen before publication is refused; one swapped
    in afterwards is replaced as a directory entry, never followed to its
    target. The Windows fallback keeps import-time and unit-test portability;
    pods are unavailable there, and the shared atomic writer still refuses
    linked parent directories and replaces rather than follows a final name.
    """
    ...
    tmp_name = f".{dst.name}.{os.getpid()}.{os.urandom(8).hex()}.tmp"
    fd = -1
    try:
        try:
        ...
        while remaining:
            ...
            if written <= 0:  # pragma: no cover - os.write either progresses or raises
                raise OSError(f"short write while publishing pod boot override {dst}")
            remaining = remaining[written:]
        os.fsync(fd)
        os.close(fd)
        fd = -1
        os.replace(tmp_name, dst.name, src_dir_fd=dir_fd, dst_dir_fd=dir_fd)
