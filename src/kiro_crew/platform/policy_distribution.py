...
def refresh_now(*, force: bool = False) -> RefreshOutcome:
    ...
    if fetched.not_modified:
        ...
        if _body_digest(cached.body) == installed:
            ...
            # to do per interval.
            try:
                # Kept, not discarded: the lapsed-grant repair below needs the central
                # document as a ceiling, and this is the same parse it would have to do.
                confirmed = parse_distributed_policy(cached.body, source=dist.source)
            except PlatformCompositionError as exc:
                return _refresh_failure(
                    REFRESH_REJECTED,
                    dist,
            ...
            # A RELEASED ROLLBACK is the one installed ceiling whose validity ends on a
            # CLOCK rather than on a publish, so for it "the document did not change"
            # stops implying "the ceiling still holds". Once the grant lapses the ladder
            # would pick the central document again -- but nothing re-ran it: the digest
            # shortcut above returns before any composition, ``parse_distributed_policy``
            # only re-checks the trust root, and there is no expiry watcher anywhere in
            # the module. So a permissive rollback stayed in effect indefinitely, an
            # override outliving its own dated window, clearable only by a restart. The
            # digest recorded on install is the CENTRAL document's while the ceiling
            # actually installed is the local one, so the shortcut cannot notice either.
            #
            # Gated on the INSTALLED tier being a grantable one. For every host that was
            # never granted break-glass that is a set-membership test on an
            # already-loaded ceiling and nothing else -- no extra read, no extra audit --
            # and it self-clears: the moment recomposition restores the central document
            # the tier stops being grantable and this never runs again. While the grant
            # is still LIVE the pre-fetch guard above has already stood the poll down, so
            # this can only fire AFTER expiry, which is exactly when an operator expects
            # the rollback to end. It also repairs a host stranded through BOOT, not just
            # through a refresh install, because both arrive at the same stranded state.
            from kiro_crew.platform.context import current_context

            installed_ceiling = current_context().governance
            if installed_ceiling is not None and installed_ceiling.tier in BREAK_GLASS_TIERS:
                from kiro_crew.platform.governance import compose_installed_ceiling

                recomposed = compose_installed_ceiling(confirmed)
                if recomposed != installed_ceiling:
                    try:
                        apply_ceiling(confirmed)
                    except Exception as exc:
                        # The rollback is lapsed but the central ceiling will not install
                        # here. Keeping the running ceiling and reporting is the same
                        # choice the install path makes: a poll must not take down a host
                        # that is already up, and the operator gets an audited reason.
                        return _refresh_failure(
                            REFRESH_REJECTED,
                            dist,
                            "the lapsed break-glass rollback could not be replaced by "
                            f"the central ceiling: {exc}",
                            incident="compose",
                        )
                    logger.warning(
                        "the break-glass grant over the %s tier has lapsed; the central "
                        "security policy governs again",
                        installed_ceiling.tier,
                    )
                    _run_post_install_hooks()
                    _audit_refresh(REFRESH_APPLIED, dist)
                    return RefreshOutcome(
                        REFRESH_APPLIED, signature_state=recomposed.signature_state
                    )
            _run_post_install_hooks()
